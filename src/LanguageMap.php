...
final class LanguageMap implements ArrayAccess, Countable
{
    ...
    public static function isValidTag($tag): bool
    {
        ...
        foreach ($parts as $part) {
            if (in_array($part, self::LANGUAGE)) {
                $count++;
                continue;
            }
            if (in_array($part, self::EXTLANG)) {
                $count++;
                continue;
            }
            if (in_array($part, self::SCRIPT)) {
                $count++;
                continue;
            }
            if (in_array($part, self::REGION)) {
                $count++;
                continue;
            }
            if (in_array($part, self::VARIANT)) {
                $count++;
                continue;
            }
            if (in_array($part, self::GRANDFATHERED)) {
                $count++;
                continue;
            }
            if (in_array($part, self::REDUNDANT)) {
                $count++;
            }
        }

        return count($parts) === $count;
    }
}
